...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight >= 2 && nHeight <= 8)
        nSubsidy = 50000000 * COIN;
    else if (nHeight >= 9 && nHeight <= 999)
        nSubsidy = 1 * COIN;
    else if (nHeight >= 1000 && nHeight <= 10000)
        nSubsidy = 50 * COIN;
    else if (nHeight >= 10001 && nHeight <= 30000)
        nSubsidy = 20 * COIN;
...
{
    RenameThread("reliefcoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TGAO: We use certain sporks during IBD, so check to see if they are
